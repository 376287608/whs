package com.ufgov.whs.company.controller;
...
public class CompanyUserController{
	...
	public @ResponseBody Map<String,String> registerCompany(SysUser user,CompanyUser company,String password,String repwd,
			String registerName,String phone,String email,String credentialsType,String credentialNumber,String companyName,
			String orgBarcode,String certificateCode) {
	...
	@ResponseBody
	public Map<String,String> ajaxUpdateUser(String registerName,String phone,String email,
			String credentialsType,String credentialNumber,String companyName) {
		//当前用户
		SysUser user = (SysUser)SecurityUtils.getSubject().getSession().getAttribute("user");
		CompanyUser companyUser = companyService.getByUserId(user.getId());
		//返回信息
		Map<String,String> info = new HashMap<String, String>();
		if(isEmpty(registerName) || isEmpty(phone) || isEmpty(email) || isEmpty(credentialsType)
				|| isEmpty(credentialNumber) || isEmpty(companyName)){
			info.put("flag", "0") ;
			info.put("message", "信息填写有误，请检查录入数据是否正确");
			return info;
		...
		companyUser.setRegisterName(registerName);
		companyUser.setPhone(phone);
		companyUser.setEmail(email);
		companyUser.setCredentialsType(credentialsType);
		companyUser.setCredentialNumber(credentialNumber);
		//更新用户资料到数据库
		if(companyService.save(companyUser.getUserId(),companyUser,false)){
			info.put("flag", "1") ;
			info.put("message", "用户信息修改成功");
		}else{
			info.put("flag", "0") ;
			info.put("message", "用户信息修改失败");
		}
	...
	public String login(Model model) {
		...
		if(user != null){
			...
			if(!user.getUserType().equals("0")){
				...
				if(!StringUtils.isBlank(loginPath) && (loginPath.indexOf("company") != -1)){
					model.addAttribute("loginError", "用户名不存在，请重试");
				}
				return "system/loginCompany";
			}else{
				return "main/index";
			}
		}
	...
	public @ResponseBody String logout(Model model,String flag) {
		Subject subject = SecurityUtils.getSubject();
		subject.logout();
		return "true";
	}
