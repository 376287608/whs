...
layui
		.use(
				...
				$("#ProcessAndBusinessBtn").click(function() {
					 ...
					 $("#checkAll").click(function() {	
						 if($(this).text()=='全选'){
							 ...
							 var rs = $('.layui-table-body:eq(1).layui-table-main table tr');				
								$.each(rs, function(index, e) {
									if($(e).has(".layui-form-checked").length<1){
										e.children[0].children[0].children[1].click();
									}
							 ...
							 var rs = $('.layui-table-body:eq(1).layui-table-main table tr');				
								$.each(rs, function(index, e) {
									if($(e).has(".layui-form-checked").length>0){
										e.children[0].children[0].children[1].click();
									}
								});
						 }   
					...
					//勾选业务数计算
						if(exportIds.has(obj.data.id)){
							newbusinessCount=newbusinessCount-1;
							$("#businessCount").text(newbusinessCount);
						}else{
							newbusinessCount=newbusinessCount+1;
							 $("#businessCount").text(newbusinessCount);
					...
					function confirmbusiness(exportIds) {	
				        	...
				        	layer.msg('选中了：'+ exportIds.size() + ' 个业务');
						    	$.ajax({
						            ...
						            data:{
						            	'id':Id,
						            	'businessIds': exportIds.toString()    			
						            },
						            success: function (data) {
						                if (data.success == 'true') {
						                	successTip(data);
